...
{{ warning("A config written before 1.0.0b3 on a non-UTF-8 console still reads back correctly only
    on the machine that wrote it: the fallback recovers the bytes with the local encoding, which is a
    different encoding elsewhere. Rewrite affected configs once, or keep their values ASCII and move
