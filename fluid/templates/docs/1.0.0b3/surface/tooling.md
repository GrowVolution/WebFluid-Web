...
{{ rule("BASE_URL defaults to http://localhost:8000. Set it before anything renders an external URL
    — a mail link, a canonical tag — because nothing warns you that the default is wrong.") }}

{{ bug("The no-op ngettext fallback installed when EXT_BABEL is off interpolates with `n`, while the
    real implementation interpolates with `num`. So ngettext('%(num)d item', '%(num)d items', n)
    renders with the battery on and raises KeyError: 'num' with it off. Write the count into the
    string if a template must work in both configurations.") }}
