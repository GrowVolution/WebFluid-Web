...
{{ rule("The vite_ns cookie is client-controlled input. Since 1.0.0b3 it is checked against the set
    of registered frontend namespaces and the resolved file must stay under project_root. Before
    that, an empty vite_ns rooted the lookup at project_root and GET /app_configs/<app>.ini returned
    the config with SECRET_KEY in it.") }}

{{ bug("The lookup resolves under <vite_ns>/<path> and <vite_ns>/public/<path> — the workspace
    directory itself, not dist/. The framework sets the cookie when it serves the Vite index, so in
    production any visitor can fetch any file inside a registered workspace (GET /package.json,
    GET /src/main.ts). Keep secrets out of frontend workspaces.") }}
