...
| `MAIL_PORT`                       | `587`                                      |                                                                     |
| `MAIL_USE_TLS`                    | `False`                                    | Implicit TLS (port 465). Honoured by both clients                   |
...
{{ rule("The two TLS defaults were swapped in 1.0.0b3 to match MAIL_PORT = 587. Setting both flags
    now raises FrameworkException at startup. For an implicit-TLS server, set MAIL_PORT = 465 AND
    MAIL_USE_TLS = True AND MAIL_USE_STARTTLS = False.") }}
