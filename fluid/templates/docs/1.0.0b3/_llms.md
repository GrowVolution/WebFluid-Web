...
If `webfluid` is installed in the repository you are working in, it also ships an agent skill at
`webfluid/.agents/skills/webfluid` — `SKILL.md` plus six reference files. That is the same material
cut for editing a codebase; these pages are the exhaustive version, and the package's own
`CHANGELOG.md` is the authority when the installed version is newer than the published latest.
...
- [SQLAlchemy]({{ docs }}{{ base }}ext/sqlalchemy.md): `db.Model`, sync and async executors, binds, driver-less URIs, engine disposal, what survives an executor block.
- [Migrate]({{ docs }}{{ base }}ext/migrate.md): the Alembic wrapper, `prepare_fluid()`, single- versus multi-database templates, SQLite batch mode, what the migration environment sees.
- [Mail]({{ docs }}{{ base }}ext/mailman.md): SMTP sync and async, the body mapping, attachments, connection reuse, the two mutually exclusive TLS modes.
- [Babel]({{ docs }}{{ base }}ext/babel.md): gettext callables, the JSON-backed runtime translation store, domains and the findtext escalation, locale/timezone selection, the nine format filters, and the two live-translation defects.
- [Security]({{ docs }}{{ base }}ext/security.md): users, roles, permissions, argon2, the full route-guard chain including the email-verified gate, CSRF, single-use tokens, OAuth, bearer grants and what they bypass, the `raise_on_sql` relationships.
- [Events]({{ docs }}{{ base }}ext/events.md): signals, events and queries, where registration belongs and what goes wrong elsewhere, the socket's request context, browser push, best-effort delivery.
...
- [Template resolution]({{ docs }}{{ base }}surface/jinja.md): autoescaping and `Markup`, the loader stack and its order, namespace prefixes, overriding framework templates, what freezing buys.
...
- [Ocean]({{ docs }}{{ base }}cli/ocean.md): searching, installing and publishing packages, bundle expansion, prerelease channel resolution.
