...
| `jinja_env`                        | The async `jinja2.Environment` — `autoescape=select_autoescape(("html","htm","xml","xhtml","svg"))` |
| `sources` / `rendered_sources`     | The frozen head sources (tuple / joined `Markup`; `Markup("")` before the freeze)                   |
