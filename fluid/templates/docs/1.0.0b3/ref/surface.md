...
| `asset_catch`       | The catch-all `/{path:path}` route that resolves a relative asset request to the workspace named by the `vite_ns` cookie. Signature `asset_catch(project_root, namespaces)`    |
| `assets.contained`  | `contained(root, *parts)` — resolves the path and returns it only if it is a file **under** `root`; `None` otherwise                                                           |
...
Since `1.0.0b3` the `vite_ns` cookie must name a **registered** frontend namespace
(`Vite._namespaces`, the set that replaced the old `_instances` counter), and every resolved path
goes through `contained()`. Before that, an empty `vite_ns` rooted the lookup at `project_root` and
`GET /app_configs/<app>.ini` returned the config, `SECRET_KEY` included.

{{ bug("The lookup still resolves under <vite_ns>/<path> and <vite_ns>/public/<path> — the frontend
    workspace directory, not dist/. The framework sets the vite_ns cookie itself when it serves the
    Vite index, so in production any visitor can fetch any file inside a registered workspace:
    GET /package.json, GET /src/main.ts. Keep nothing secret inside a frontend workspace.") }}
