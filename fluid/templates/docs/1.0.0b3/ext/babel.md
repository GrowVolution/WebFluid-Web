...
{{ rule("Use SCREAMING_SNAKE keys, not English sentences, for any catalog you control. A key that
    is not a sentence in any language cannot be confused with one, reads the same in a diff, and
    survives an editor rewording the English. The framework's own catalog does exactly this.") }}
...
A lookup walks, in order, until a domain **has** the message:
...
If none has it, the source string is returned unchanged.

Since `1.0.0b3` "has" means exactly that. `MergedTranslations` answers through a `findtext` family —
`findtext`, `nfindtext`, `pfindtext`, `npfindtext` and their four `a`-prefixed pendants — which
return `None` on a miss, checking the database first and then probing the compiled catalog's
`_catalog` for the key shape gettext uses (`msgid`, `(msgid, 0)`, `ctxmsgid`,
`(ctxmsgid, 0)`). The eight `gettext` methods are unchanged and still answer with the source
string on a miss.

{{ info("Before 1.0.0b3 the escalation accepted an answer only when it DIFFERED from the input, so a
    deliberate translation equal to its source — an English catalog translating Save to Save — was
    read as a miss and the next domain answered instead. If you worked around that, the workaround
    can go.") }}

`I18nKey` carries a `(key, domain)` unique constraint since `1.0.0b2`, so two domains can translate
the same source string differently. Beta 1 had a global unique index on `key` — upgrading from it
needs a migration.
...
Two request types on that socket, and they behave differently:

| Type        | What it does                                        | Locale it uses                                     |
|-------------|-----------------------------------------------------|----------------------------------------------------|
| `cache`     | Returns the whole catalog for a locale, cached client-side | Resolved from `data.locale` → `?lang=` → cookie → `Accept-Language` → default |
| `translate` | Translates one message server-side (`live.*`)       | **Always `BABEL_DEFAULT_LOCALE`** — see below      |

```js
const t = new window.wf.ext.i18n.Translations()   // cache-backed: correct
await t._("HOME_TITLE")

await t.live._("HOME_TITLE")                       // socket round trip: default locale only
```

{{ bug("The translate handler runs without a FluidContext, so get_locale() has nothing to read and
    falls back to BABEL_DEFAULT_LOCALE on every call — while its sibling cache handler resolves the
    connection's locale correctly. The two halves of the same socket disagree.") }}

{{ bug("live._p sends args [string, context] where the server reads apgettext(context, string), and
    live._np sends [singular, plural, num, context] against anpgettext(context, singular, plural,
    num). Both come back wrong even in the default locale. The cache-backed _p / _np are correct.") }}

{{ rule("Use the cache-backed client API. It is one round trip per locale instead of one per string,
    it answers in the visitor's language, and it is the half that works.") }}
