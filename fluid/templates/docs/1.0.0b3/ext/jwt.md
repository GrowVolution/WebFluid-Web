...
{{ warning("What that closes: kid: \"current\" used to resolve to the cache entry jwt:current, whose
    value is the key id itself — a uuid published in the header of every token the app issues. Anyone
    holding one valid token could read it and sign their own tokens with it, for any sub and any
