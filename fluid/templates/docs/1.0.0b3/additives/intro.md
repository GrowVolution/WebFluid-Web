...
All of these are **scoped to the Additive's own routes**, not the whole app — and, for the two
request hooks, to the `api` and `app` routers only.
...
with anyone else's. Like the framework one, it works off-request since `1.0.0b3`: it falls back to
the application's route table and `external=True` prefixes `BASE_URL`.

{{ bug("additive.before_request / after_request do not run for additive.ws routes. The request
    lifecycle is installed as http_middleware on api and app; websocket routes are registered
    without it. A guard written as a before_request does not protect your Additive's websockets —
    check the connection inside the endpoint.") }}
