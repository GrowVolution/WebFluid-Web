...
current_user            session lookup, yields User | None
  └── require_user      + CSRF check on unsafe methods, 401 if no user
...
Two consequences: **every** role or permission guard also enforces authentication, CSRF and 2FA;
and `require_2fa` is a no-op for a user who has no second factor at all.
...
comes from `SECURITY_CSRF_COOKIE_NAME` and is honoured by both halves as of this release.
...
{{ bug("resolve_bearer looks the principal up with int(sub). A token whose sub is a uuid or an email
    raises ValueError inside the gate's fallback branch, which nothing catches, and the request
    comes back 500 instead of 401. Mint tokens with the user's primary key in sub.") }}
