...
{{ bug("database_uris() inserts the driver with a plain string replace, so *every* occurrence of
    the scheme word in the URI is rewritten, not only the leading one.
    sqlite:///data/sqlite/app.db becomes sqlite+aiosqlite:///data/sqlite+aiosqlite/app.db, and a
    MySQL database literally named mysql_prod gets the same treatment. Keep the scheme word out of
    paths, database names, users and passwords.") }}
