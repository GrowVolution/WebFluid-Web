...
**Autoescape is off.** Values are inserted verbatim.

{{ rule("Escape untrusted values yourself: {{ value | e }}. Or, if a value is known-safe HTML, wrap
    it in markupsafe.Markup. The framework relies on this — sources, theme links and frontend()
    output are all Markup, and turning autoescape on would break them.") }}
