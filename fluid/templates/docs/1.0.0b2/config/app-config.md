...
`wf run <name>` looks for `app_configs/<name>.ini`, parses it with `configparser`
(`optionxform = str`, so keys keep their case), flattens **every section** into one environment
