...
|------------------------------------------|-----------------|-------------------------------------------------------|
| `url_for(name, **params)` (Jinja global) | **yes**         | path, or absolute with `external=True`                |
...
{{ bug("The url_for Jinja global is built from the request in the current context and is None when
    there is none. A template rendered from a startup hook, a scheduled job or a mail routine fails
    with 'NoneType is not callable'. Use fluid.url_path_for() or an absolute BASE_URL there.") }}
