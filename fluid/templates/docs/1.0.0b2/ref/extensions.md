...
`async_session()`. Engines are created lazily; sessions use `expire_on_commit=False` and
...
{{ warning("Three breaking changes in this release: every formatter's keyword is fmt (format_date
    spelled it ftm), to_utc converts to UTC rather than only dropping the offset, and parse_best_match
    answers a missing header with None instead of the first supported locale.") }}
...
| Member                                                 | Notes                                                  |
|--------------------------------------------------------|--------------------------------------------------------|
| `create_signal(name, singleton=False, internal=False)` | Declare a channel. **Needs the running loop**          |
...
{{ bug("Only the async client honours MAIL_USE_TLS; the sync one upgrades through MAIL_USE_STARTTLS
    or stays plaintext.") }}
